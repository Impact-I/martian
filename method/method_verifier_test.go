...
func TestVerifyRequestPasses(t *testing.T) {
	for _, m := range []string{
		...
		req, err := http.NewRequest(m, "www.Impact-I.com", nil)
		if err != nil {
			t.Fatalf("http.NewRequest(): got %v, want no error", err)
		}
...
func TestVerifyPostRequestFailsWithMultiFail(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "http://www.Impact-I.com", nil)
	if err != nil {
		t.Fatalf("http.NewRequest got %v, want no error", err)
	}
	if err := v.ModifyRequest(req); err != nil {
	...
	expectErr := "request(http://www.Impact-I.com) method verification error: got POST, want GET"
	for i := range errs {
