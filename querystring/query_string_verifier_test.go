...
import (
	...
	"github.com/Impact-I/martian/v3/parse"
...
func TestVerifyRequestPasses(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "http://www.Impact-I.com?foo=baz&foo=bar", nil)
	if err != nil {
...
func TestVerifyEmptyValue(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "http://www.Impact-I.com?foo=bar", nil)
	if err != nil {
		t.Fatalf("http.NewRequest(): got %v, want no error", err)
	}
	_, remove, err := martian.TestContext(req, nil, nil)
...
func TestFailureWithMissingKey(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "http://www.Impact-I.com?fizz=bar", nil)
	if err != nil {
		t.Fatalf("http.NewRequest(): got %v, want no error", err)
	}
	_, remove, err := martian.TestContext(req, nil, nil)
	...
	expectErr := "request(http://www.Impact-I.com?fizz=bar) param verification error: key foo not found"
	for i := range errs {
...
func TestFailureWithMultiFail(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "http://www.Impact-I.com?foo=baz", nil)
	if err != nil {
		t.Fatalf("http.NewRequest(): got %v, want no error", err)
	}
	_, remove, err := martian.TestContext(req, nil, nil)
	...
	expectErr := "request(http://www.Impact-I.com?foo=baz) param verification error: got baz for key foo, want bar"
	for i := range errs {
		if got, want := errs[i].Error(), expectErr; got != want {
			t.Errorf("%d. err.Error(): mismatched error output\ngot: %s\nwant: %s", i,
				got, want)
